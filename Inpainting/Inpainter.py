...
#print("CUDA доступен:", np.cuda.is_available())
...
class Inpainter():
    ...
    def inpaint(self):
        ...
        while stay:
            ...
            # cv2.imwrite("confidenceImage.jpg", self.confidence)

            images.append(cv2.cvtColor(self.workImage, cv2.COLOR_BGR2RGB))

        self.result = np.copy(self.workImage)
        #cv2.imshow("Confidence", self.confidence)
        # cv2.imwrite("confidenceImage.jpg", self.confidence)

        imageio.mimsave('inpainting_process.gif', images, duration=5)
    ...
    def computeTarget(self):
        ...
        for i in range(len(self.fillFront)):
            ...
            # Way 2
            Rnp = (1-omega) * self.confidence[y, x] + omega
            priority = alpha * Rnp + beta * self.data[y, x]
